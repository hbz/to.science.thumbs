...
public class ThumbnailGenerator {
    ...
    /**
     * @param ts the actual content to create a thumbnail from
     * @param contentType the MediaType for the content
     * @param size acually the width
     * @param name 
    ...
    public File createThumbnail(InputStream ts, MediaType contentType, int size, String name) throws IOException {
        ...
        try {
            ...
            log.warn("", e);
            result = this.generateThumbnailFromImage(props.getPathToDefaultPic().getInputStream(), size, "png",name);
        }
        return result;
    }
    ...
    private File generateMimeTypeImage(MediaType contentType, int size,String name) throws IOException {
        ...
        try {
            if (contentType.is(MediaType.ANY_AUDIO_TYPE))
                result = this.generateThumbnailFromImage(props.getPathToAudioPic().getInputStream(), size, "png", name);
            else if (contentType.is(MediaType.ANY_IMAGE_TYPE))
                result = this.generateThumbnailFromImage(props.getPathToImagePic().getInputStream(), size, "png", name);
                ...
                result = this.generateThumbnailFromImage(props.getPathToTextPic().getInputStream(), size, "png", name);
            else if (contentType.is(MediaType.ANY_VIDEO_TYPE))
                result = this.generateThumbnailFromImage(props.getPathToVideoPic().getInputStream(), size, "png", name);
            else if (contentType.is(MediaType.ZIP))
                result = this.generateThumbnailFromImage(props.getPathToZipPic().getInputStream(), size, "png", name);
            else if (contentType.is(MediaType.PDF))
                result = this.generateThumbnailFromImage(props.getPathToPdfPic().getInputStream(), size, "png", name);
            else
                result = this.generateThumbnailFromImage(props.getPathToDefaultPic().getInputStream(), size, "png", name);
        } catch (Throwable e) {
            log.warn("", e);
